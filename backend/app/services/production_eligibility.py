...
async def find_assignment(db: AsyncSession, job: PlateJob) -> ProductionAssignment | None:
    ...
    for profile_id in profile_ids:
        ...
        if candidate_printers:
            # Once a real printer of the requested model is configured, do
            # not silently route a production order to a virtual test target
            # merely because the real device is busy, offline, or mismatched.
            # The job must remain draft until the real printer is usable.

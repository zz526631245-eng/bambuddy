...
export function ProductionOrderDetailPage(){
 ...
 const state=useMutation({mutationFn:(action:'pause'|'resume'|'cancel')=>productionApi.changeStatus(id,action),onSuccess:()=>{refresh();qc.invalidateQueries({queryKey:['production-orders']});}});
 ...
 return <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
  ...
  <div className="flex flex-wrap justify-between gap-4"><div><div className="font-mono text-bambu-green">{order.order_number}</div><h1 className="text-3xl text-white font-bold">{String(order.product_snapshot?.name||'生产订单')}</h1><p className="text-bambu-gray">生产 {order.quantity} 套 · 优先级 {order.priority} · {statusText[order.status]||order.status}</p></div><div className="flex gap-2">{order.status==='planned'&&<Button variant="secondary" onClick={()=>state.mutate('pause')}><Pause size={16}/>暂停</Button>}{order.status==='paused'&&<Button onClick={()=>state.mutate('resume')}><Play size={16}/>恢复</Button>}{!['cancelled','completed'].includes(order.status)&&<Button variant="danger" onClick={()=>window.confirm('确认取消这个订单？')&&state.mutate('cancel')}><Ban size={16}/>取消</Button>}</div></div>

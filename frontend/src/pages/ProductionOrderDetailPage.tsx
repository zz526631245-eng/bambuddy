...
import { AlertTriangle, ArrowLeft, Ban, CalendarDays, CheckCircle2, Download, Pause, Play, Plus, RefreshCw, ScrollText, Trash2, XCircle } from 'lucide-react';
...
const operationText: Record<string, string> = {
  ...
  production_order_merged: '合并到现有批次', production_order_quantity_added: '追加生产数量',
  production_order_replanned: '重新规划交期', production_order_deleted: '删除并保留历史',
};
...
function SliceJobRow({
  ...
  const isRealAssigned = job.assigned_printer_id != null && job.virtual_printer_id == null;
  ...
  return <div className="rounded-lg border border-bambu-dark-tertiary p-3 space-y-2">
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {job.slice_status === 'failed' && !isRealAssigned && <Button type="button" variant="secondary" size="sm" onClick={() => onRetry(job.id)}><RefreshCw size={15} />重试模拟切片</Button>}
      {job.slice_status !== 'slicing' && <Button type="button" size="sm" onClick={() => onRealSlice(job.id)}><RefreshCw size={15} />{isRealAssigned ? '重试真实切片' : '运行真实切片'}</Button>}
...
export function ProductionOrderDetailPage() {
  ...
  const [replanDueAt, setReplanDueAt] = useState('');
  const [replanPriority, setReplanPriority] = useState<number | null>(null);
  ...
  const appendOrder = useMutation({ mutationFn: () => productionApi.appendOrderQuantity(id, { quantity: appendQuantity }), onSuccess: () => { refresh(); queryClient.invalidateQueries({ queryKey: ['production-orders'] }); setAppendQuantity(1); } });
  const replanOrder = useMutation({ mutationFn: () => productionApi.replanOrder(id, { due_at: replanDueAt ? new Date(replanDueAt).toISOString() : null, ...(replanPriority == null ? {} : { priority: replanPriority }) }), onSuccess: () => { refresh(); queryClient.invalidateQueries({ queryKey: ['production-orders'] }); } });
  ...
  const overdue = order.overdue || order.delivery_status === 'overdue';
  return <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
    ...
    <div className="flex flex-wrap justify-between gap-4"><div><div className="font-mono text-bambu-green">{order.order_number}</div><h1 className="text-3xl text-white font-bold">{String(order.product_snapshot?.name || '生产订单')}</h1><p className="text-bambu-gray">生产 {order.quantity} 套 · 优先级 <span className="text-white">{order.priority_label || priorityText[order.priority] || '极低'}</span> · {statusText[order.status] || order.status}</p><p className={`mt-2 flex items-center gap-2 text-sm ${overdue ? 'text-red-300' : 'text-bambu-gray'}`}><CalendarDays size={15} />交期：{order.due_at ? new Date(order.due_at).toLocaleString('zh-CN') : '未设置'}{overdue && <><AlertTriangle size={15} />已逾期，需要重新规划</>}</p></div><div className="flex flex-wrap gap-2">{order.status === 'planned' && <Button variant="secondary" onClick={() => state.mutate('pause')}><Pause size={16} />暂停</Button>}{order.status === 'paused' && <Button onClick={() => state.mutate('resume')}><Play size={16} />恢复</Button>}{!['cancelled', 'completed'].includes(order.status) && <Button variant="danger" onClick={() => window.confirm('确定取消这个订单吗？') && cancelOrder.mutate()}><Ban size={16} />取消整个打印任务</Button>}<Button variant="secondary" onClick={() => window.confirm('确定删除整个生产订单及其打印任务吗？') && deleteOrder.mutate()}><Trash2 size={16} />删除整个打印任务</Button></div></div>
    {(deleteOrder.error || cancelOrder.error || cancelJob.error || deleteJob.error || workflow.error || appendOrder.error || replanOrder.error) && <p role="alert" className="text-red-400">{String((deleteOrder.error || cancelOrder.error || cancelJob.error || deleteJob.error || workflow.error || appendOrder.error || replanOrder.error) as Error)}</p>}
    <section className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">{[['目标', order.quantity], ['已完成', order.completed_quantity ?? 0], ['打印中', order.printing_quantity ?? 0], ['待质检', order.quality_quantity ?? 0], ['待清板', order.cleanup_quantity ?? 0], ['剩余', order.remaining_quantity ?? order.quantity]].map(([label, value]) => <Card key={String(label)}><CardContent className="p-4"><div className="text-xs text-bambu-gray">{label}</div><div className="mt-1 text-2xl font-semibold text-white">{value}</div></CardContent></Card>)}</section>
    <Card><CardContent className="py-4"><div className="flex flex-wrap items-center gap-2 text-sm">{['已分配','待打印','打印中','待质检','待清板','已完成'].map((label,index) => <div key={label} className="flex items-center gap-2"><span className="rounded-full bg-bambu-dark-secondary px-3 py-1 text-white">{label}</span>{index < 5 && <span className="text-bambu-gray">→</span>}</div>)}</div><p className="mt-2 text-sm text-bambu-gray">真实打印机由系统按型号和已扫码耗材自动匹配；切片成功且打印机可用后会自动进入现有打印队列。</p></CardContent></Card>
    {!['cancelled', 'completed'].includes(order.status) && <Card><CardContent className="flex flex-wrap items-end gap-4"><div><div className="mb-2 flex items-center gap-2 text-sm font-semibold text-white"><Plus size={16} className="text-bambu-green" />追加到当前批次</div><div className="text-xs text-bambu-gray">不会创建新的订单号，追加数量会记入本批次操作记录。</div></div><input aria-label="详情追加数量" type="number" min={1} value={appendQuantity} onChange={event => setAppendQuantity(Number(event.target.value))} className="w-28 rounded-lg border border-bambu-dark-tertiary bg-bambu-dark px-3 py-2 text-white" /><Button type="button" disabled={appendOrder.isPending || appendQuantity < 1} onClick={() => appendOrder.mutate()}>确认追加</Button><div className="ml-auto flex flex-wrap items-end gap-2"><label className="text-xs text-bambu-gray">新交期<input aria-label="重新规划交期" type="datetime-local" value={replanDueAt} onChange={event => setReplanDueAt(event.target.value)} className="mt-1 block rounded-lg border border-bambu-dark-tertiary bg-bambu-dark px-3 py-2 text-white" /></label><label className="text-xs text-bambu-gray">优先级<select aria-label="重新规划优先级" value={replanPriority ?? order.priority} onChange={event => setReplanPriority(Number(event.target.value))} className="mt-1 block rounded-lg border border-bambu-dark-tertiary bg-bambu-dark px-3 py-2 text-white"><option value={4}>最高</option><option value={3}>高</option><option value={2}>中</option><option value={1}>低</option><option value={0}>极低</option></select></label><Button type="button" variant="secondary" disabled={!replanDueAt || replanOrder.isPending} onClick={() => replanOrder.mutate()}>重新排期</Button></div></CardContent></Card>}

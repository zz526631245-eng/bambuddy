...
function PrinterStatusCard({ item }: { item: ProductionPrinterStatus }) {
  ...
  return <Card className="h-full">
    <CardContent className="space-y-4">
      ...
      <div className="space-y-2 border-t border-bambu-gray-dark pt-3">
...
export function ProductionPrinterStatusPage() {
  ...
  return <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
    <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 p-3 text-sm text-amber-200">第13阶段状态监控与故障演练入口：这里使用模拟心跳验证在线、离线、故障和维修状态，不连接真实打印机、不发送打印。第14阶段接入真实适配器时仍使用同一个心跳接口。</div>

...
阶段 11 已完成；阶段 12 已在分支 codex/feature-stage12-direct-consumable-scan 完成直供耗材绑定、扫码自动替换、当前耗材快照同步、颜色匹配约束、摄像头/原生相机扫码入口和材料二维码生成下载。第 11 阶段演示产品、虚拟打印机和本地数据库仅用于验收，不在代码或安装包中。仍未连接真实打印机、读取真实状态或发送打印。最新阶段 12 结论见 docs/STAGE12_HANDOFF_CN.md。

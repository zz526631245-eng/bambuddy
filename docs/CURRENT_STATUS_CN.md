...
### 菜单文字与产品库尺寸修正（2026-08-12）
- 恢复左侧主菜单的完整文字入口，不再因为低频入口策略隐藏库存、队列、文件、耗材和生产状态等菜单项；原有权限控制和用户主动隐藏设置继续有效。
- 产品库页面容器宽度与生产工作台对齐至 `1500px`，避免产品卡片和内容区相对其他桌面页面显得缩小。
- 前端 Lint、定向测试和构建已重新验证；未构建安装包。

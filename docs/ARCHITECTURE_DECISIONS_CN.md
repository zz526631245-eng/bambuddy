...
## AD-025：真实生产自动匹配并自动进入既有队列

阶段 14 的生产订单确认和后台恢复循环会自动刷新已连接真实打印机的状态，并优先按打印机型号、扫码耗材材料/颜色和队列占用情况创建真实 `PrintQueueItem`；同型号真实打印机已经配置但暂时离线、忙碌或耗材不匹配时，订单保持待分配，不会静默落到虚拟测试机。只有没有任何同型号真实设备的显式软件测试场景才保留虚拟回退。阶段 11 的虚拟配置位置和静态能力列表不能覆盖真实打印机的扫码耗材快照。真实任务使用现有切片 sidecar 生成与目标型号绑定的 `.gcode.3mf`；切片成功、打印机在线且空闲后，系统自动取消源文件占位队列并把真实切片产物交给既有队列，队列继续负责 FTP/MQTT 发送。切片产生多个产物时暂不自动替换单个队列占位项，避免只发送第一盘造成少打；这类任务需要先完成多盘队列建模。

数量拆分时，切片器的 `plate` 参数表示源 3MF 盘索引而不是成品数量索引；重复使用单盘源文件必须传入盘 0。原生 Windows 的本地 sidecar 地址统一使用 IPv4 回环，远程/Docker 主机名保持不变。每次重排队/重新分配都写入唯一操作事件，避免 SQLite 重试时的唯一键冲突。

## AD-026：自动摆盘只改变位置和 Z 轴朝向，保留源文件参数

自动摆盘计算前必须先应用 3MF 的组件变换和 build-item 变换，使用变换后的真实 XY 投影和高度规划容量；不能只读取网格顶点的原始 X/Y。写回 3MF 时只允许更新 build-item 的 XY 平移和绕世界 Z 轴的 0/90/180/270 度朝向，保留原始 3x3 矩阵中的缩放、X/Y 倾斜、Z 高度和所有 `project_settings.config`、对象级、支撑、填充、温度参数。切片器仍按源文件参数工作，打印机切换只通过既有的目标打印机身份字段完成。

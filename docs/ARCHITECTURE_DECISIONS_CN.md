...
## AD-025：真实生产自动匹配，发送仍由人工确认

阶段 14 的生产订单确认和后台恢复循环会自动刷新已连接真实打印机的状态，并优先按打印机型号、扫码耗材材料/颜色和队列占用情况创建真实 `PrintQueueItem`；同型号真实打印机已经配置但暂时离线、忙碌或耗材不匹配时，订单保持待分配，不会静默落到虚拟测试机。只有没有任何同型号真实设备的显式软件测试场景才保留虚拟回退。阶段 11 的虚拟配置位置和静态能力列表不能覆盖真实打印机的扫码耗材快照。真实任务使用现有切片 sidecar 生成与目标型号绑定的 `.gcode.3mf`，但仍保持队列安全暂停和显式人工发送确认，不自动下发打印命令。

...
```powershell
python -m pytest backend/tests/integration/test_stage13_printer_status.py -q
```

预期：2 项通过，覆盖虚拟打印机状态列表、幂等心跳、故障/维修不可分配和过期心跳自动离线。

人工入口：`https://127.0.0.1:8019/production-printer-status`。页面每 3 秒刷新，可以对虚拟打印机发送空闲、打印中、暂停、离线、故障和维修中心跳。阶段 13 不连接真实打印机；状态不可用时，新的生产任务不会分配到该目标，尚未开始的已分配任务会回到待分配。

## 阶段 14 单台真实打印机受控验收

> 这一节会在最后一步把一份真实切片文件交给已连接的打印机。请先使用不重要的测试模型、在打印机旁完成验收；不要用虚拟打印机或未切片的源文件。

1. 在原有“打印机”页面确认一台真实打印机已启用、已填写本机数据库中的访问码，并且应用已显示 MQTT 连接成功；在“打印机直供耗材”中确认其当前耗材的材料和颜色与测试产品相符。
2. 打开 `/production-printer-status`。真实机只能显示 MQTT 自动读取的状态，不能发送模拟心跳；虚拟机仍可单独做软件演练。
3. 创建一个测试生产订单并确认盘任务。自动分配会优先使用符合型号、位置和材料/颜色条件的真实机；仅在没有真实可用机时才回退到虚拟打印机。
4. 完成真实切片后打开 `/slice-library`，在目标切片卡片点击“发送到真实打印机”，从列表明确选择那一台真实机。系统不会预选或自动切换机器。
5. 阅读确认弹窗中的文件名、打印机和提示后点击确认。此时才会把该 `.gcode.3mf` 交给 Bambuddy 原有队列；此后由原有 FTP/MQTT 上传并启动。刷新页面，确认盘任务状态依次显示“待打印 → 打印中”。
6. 打印结束后，等待 MQTT 回写为“待质检”。人工录入合格数量/报废数量，再确认清板；只有这两个后端工作流完成后，订单数量才会入账且打印机释放。
7. 再做一次失败演练：让打印任务结束为失败，确认任务仍进入“待质检”，由人工录入 0 合格数量并清板。不要通过删除队列或手工改数据库来跳过这个步骤。

以下情况属于保护性拒绝，并且不会发送打印：未点击确认、虚拟盘任务、切片文件没有关联盘任务、打印机未连接/离线、同一台真实机已有未结束生产任务，或该任务尚未替换为真实切片文件。
